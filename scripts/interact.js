...
async function main() {
    console.log('Getting the test token contract...\n');
    ...
    console.log(`Total Supply in TES: ${ethers.utils.formatUnits(totalSupply, decimals)}\n`);
    ...
    await testToken.transfer(recipientAddress, ethers.utils.parseUnits(transferAmount.toString(), decimals));
    console.log('Transfer completed');
    ownerBalance = await testToken.balanceOf(ownerAddress);
    console.log(`Balance of owner (${ownerAddress}): ${ethers.utils.formatUnits(ownerBalance, decimals)} ${symbol}`);
    let recipientBalance = await testToken.balanceOf(recipientAddress);
    ...
    const signerContract = testToken.connect(signers[1]); // Creates a new instance of the contract connected to the recipient
    ...
    let allowance = await testToken.allowance(recipientAddress, ownerAddress);
    ...
    await testToken.transferFrom(recipientAddress, ownerAddress, ethers.utils.parseUnits(transferFromAmount.toString(), decimals));
    ownerBalance = await testToken.balanceOf(ownerAddress);
    console.log(`New owner balance (${ownerAddress}): ${ethers.utils.formatUnits(ownerBalance, decimals)} ${symbol}`);
    recipientBalance = await testToken.balanceOf(recipientAddress);
    console.log(`New recipient balance (${recipientAddress}): ${ethers.utils.formatUnits(recipientBalance, decimals)} ${symbol}`);
    allowance = await testToken.allowance(recipientAddress, ownerAddress);
